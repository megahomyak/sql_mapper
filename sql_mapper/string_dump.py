...
class BaseFormattableString(ABC):
    ...
    def reformat_query(
        ...
        question mark with something like (?,?,?) (depends on the database
        connector).

        So,

        old query:
            "INSERT INTO a VALUES ?", instance_of_a
        new query:
            "INSERT INTO a VALUES (?, ?, ...)", values_from_instance_of_a
        """
        pass
    ...
    def _new_query_parameter_marks_generator(
        ...
        for argument in old_arguments:
            if isinstance(argument, ModelBase):
                new_arguments.extend(argument.values())
                yield (
                    "(" + ",".join(
                        new_parameter_mark
                        for _ in range(argument.fields_amount())
                    ) + ")"
                )
            else:
